...
<template>
  <div class="pt-[var(--header-height)] min-h-screen">
    <div class="mx-auto max-w-4xl px-6 py-16">
      <div v-if="orderId" class="text-center py-24">
        <p class="text-xs uppercase tracking-widest text-brand-500 mb-4">Order confirmed</p>
        <h1 class="font-display text-4xl text-brand-900 mb-4">Thank you</h1>
        <p class="text-brand-600 mb-2">Your order number is <strong>{{ orderId }}</strong></p>
        <p class="text-brand-500 text-sm mb-10">
      ...
      <template v-else>
        <h1 class="font-display text-4xl text-brand-900 mb-12">Checkout</h1>

        <div class="grid lg:grid-cols-5 gap-12">
          <form class="lg:col-span-3 space-y-6" @submit.prevent="submit">
            <div v-for="field in fields" :key="field.key">
              <label :for="field.key" class="block text-xs uppercase tracking-widest text-brand-500 mb-2">
                {{ field.label }}
              </label>
              <input
                :id="field.key"
              ...
              />
              <p v-if="errors[field.key]" class="text-red-600 text-xs mt-1">{{ errors[field.key] }}</p>
            </div>
          ...
          <div class="lg:col-span-2 bg-brand-100 p-8 h-fit">

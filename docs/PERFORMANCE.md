...
生产环境：设置 `NUXT_PUBLIC_ENABLE_ANALYTICS=true`（transport 层 Phase 2 接入）
...
\* 移动端 medium tier：`parallax` 取决于 `motionCapabilities` 与 `NUXT_PUBLIC_ENABLE_PARALLAX`
...
| 路由切换卡顿 | 未 destroy   | 检查 `layouts/default.vue` watch |

## 6. 线上运维

- CI 每次 push 自动 lint + test + build
- 静态资源 CDN 缓存 30 天
- 环境变量动态关闭动画：`NUXT_PUBLIC_ENABLE_SMOOTH_SCROLL=false`
